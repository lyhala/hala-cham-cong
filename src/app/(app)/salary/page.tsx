...
function Slip({ month, data: d, visible }: { month: string; data: PayrollResult; visible: Record<string, boolean> }) {
  const plus: [string, string, boolean][] = [
    ...
    ["Công bù", `${d.bonusUnits ?? 0} công`, visible.bonusUnits && (d.bonusUnits ?? 0) > 0],
    ["Phép tồn quy đổi", `${d.leaveDaysPaidOut ?? 0} ngày`, visible.leavePayout && (d.leaveDaysPaidOut ?? 0) > 0],
    ["Tổng công", `${d.totalUnits} / ${d.standardWorkDays}`, visible.workUnits && d.totalUnits !== d.actualWorkUnits],
